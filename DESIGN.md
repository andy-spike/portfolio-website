...
The visual world is a **foundry sheet**: the artifact a foundry prints so a professional can judge a working object at a glance and in depth. Every Portfolio Project is set on it as one object shown at several magnifications, measured, annotated, and put to use.

It was chosen because the reader's job and the form's job are the same job. It also does two things no other candidate did: the sheet's **alternates** — the glyphs considered and not set — are exactly the Rejected Alternatives that carry every Case Study, and the sheet's **reversed plate** makes a dark rendition native to the form rather than a toggle bolted onto a light one.
...
5. **Spanish falls back, never 404s.** A missing or `draft: true` Spanish project renders the English setting behind a notice.
...
The light ground is a neutral paper at very low chroma — deliberately not cream. Cream plus ink is the predictable rendition of any paper world, and the sheet's authority comes from precision, not warmth.

## Type

Two families, both variable, both self-hosted through Fontsource. The width axis is loaded (`wdth.css`), not just weight — the sheet's signature gesture depends on it.

- **Archivo Variable** (`wght` 100–900, `wdth` 62–125) — display and body. A grotesque with enough width range to demonstrate itself, which is what a foundry sheet exists to do.
...
| `--text-plate` | `clamp(2.125rem, 6vw, 4.25rem)` | Project title on a sheet |
...
Sheet primitives are defined in `@layer components` in `src/styles/global.css`.
...
| `.project-row` / `.project-name` | One catalogued project in the table — title, description, meta, and stack tags all inside one interactive strip |
...
- `/` and `/es/` — the sheet: showing, key, actions, project table, shown-in-use, colophon.
...
- Content lives in `src/content/projects/<locale>/<slug>.mdx`. Frontmatter carries facts; the body carries Problem → Decisions → Result. `<Decision>` is injected via the `components` prop, so authors do not import it.
- Locale resolution and English fallback are centralised in `src/lib/projects.ts`. UI strings are in `src/i18n/ui.ts`.
...
| Terminal Cast | `public/casts/dolphin.cast` | `InUse.astro` renders a reserved plate until a `cast` is set on the project. |
